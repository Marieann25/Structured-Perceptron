...
/**
 * Implementation of Structured Perceptron algorithm, you can find the detailed information about the 
 * algorithm here (http://lxmls.it.pt/2011/slides/day3_slides.tar.gz)
 */

public class StructuredPerceptron {

	/**
 	 * There are several steps for this algorithm.
 	 * 1. read data and feature
 	 * the 6301 iteration
 	 * tp: 1832   guessEntities: 1832   trueEntitites: 1832
 	 * precision = 1.0
 	 * recall = 1.0
 	 * F1 = 1.0
 	 * tp: 1832   guessEntities: 1832   trueEntitites: 1832
	...
	/**
 	 * train the structured preceptron
 	 * There are no bias term in the structured perceprton
 	 */ 
	public static double[][] learnPerceptron(List<Datum> trainData, List<Datum> testDataWithMultiplePrevLabels) {
		...
		while (run){
			...
			for (int j = 0; j < clonerTestData.size(); j++) {
				...
				//System.out.println("true label: " + testData.get(j).label + "   guess label:" + guessLabel);
