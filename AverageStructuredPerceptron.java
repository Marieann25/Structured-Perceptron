...
/**
 * Implementation of Average Structured Perceptron Algorithm, you can find
 * the detailed information about strctured perceptron with averaging 
 * through this link {@link http://www.cs.columbia.edu/~mcollins/courses/6998-2012/lectures/lec5.1.pdf}
 * <p>
 *
 * <b>NOTE: </b> we use the average weight to make prediction
 * w = w_{a} / NT , in our case, set n = 1, because there is just one sequence 
 */     
public class AverageStructuredPerceptron {

	/**
 	 * The difference between the structured perceptron 
 	 * and the structured perceptron with averaging:
 	 * (1) structured perceptron: the stopping condition is whether the predicted structure is equal to 
 	 * the gold structure
 	 * (2) structured perceptron: the stopping condition is the iteration number.       
 	 */  
	...
	/**
 	 ...
 	 */  
	public static double[][] learnPerceptron(List<Datum> trainData, List<Datum> testDataWithMultiplePrevLabels) {

                FeatureRepresentation obj = new FeatureRepresentation(trainData);
                double[][] weight = new double[obj.labelIndex.size()][obj.featureIndex.size()];
		double[][] averageWeight = new double[obj.labelIndex.size()][obj.featureIndex.size()]; 
		...
		for (int i = 0; i < iteration; i++) {
			...
			double[][] goldFeature = getStructureFeature(testData, obj.featureIndex, obj.labelIndex);
                        double[][] predictFeature = getStructureFeature(predictData, obj.featureIndex, obj.labelIndex);
		...
		Viterbi viterbi = new Viterbi(obj.labelIndex, obj.featureIndex, averageWeight);
	...
	public static boolean matrixEqual(double[][] goldFeature, double[][] predictFeature) {
                boolean equal = true;
                boolean jump = false;
	...
	public static double[][] updateWeight(double[][] weight, double[][] goldFeature, double[][] predictFeature) {
                for (int i=0; i<weight.length; i++) {
                        for (int j = 0; j < weight[i].length; j++) {
                                weight[i][j] = weight[i][j] + goldFeature[i][j] - predictFeature[i][j];
                        }
                }
                return weight;
        }
	...
	//
	public static double[][] addWeight(double[][] weight, double[][] averageWeight) {
	...
	public static double[][] getStructureFeature(List<Datum> datas, Index featureIndex, Index labelIndex) {
                double[][] structureFeature = new double[labelIndex.size()][featureIndex.size()];
